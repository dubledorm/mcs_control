...
class CollateBaseService
  ...
  def call
    ...
    new_object_list = (there_object_list - here_object_list)
    ...
    (here_object_list - there_object_list).each do |here_object|
      set_object_db_status(here_object, :only_here_exists)
    end

    # Проставляем состояние :everywhere_exists
    (here_object_list & there_object_list).each do |everywhere_object|
